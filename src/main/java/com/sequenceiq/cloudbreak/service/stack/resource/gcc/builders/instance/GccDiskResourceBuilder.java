...
public class GccDiskResourceBuilder extends GccSimpleInstanceResourceBuilder {
    ...
    public List<Resource> create(GccProvisionContextObject po, int index, List<Resource> resources, TemplateGroup templateGroup, String region) throws Exception {
        ...
        if (execute.getHttpErrorStatusCode() == null) {
            Compute.ZoneOperations.Get zoneOperations = createZoneOperations(po.getCompute(), credential, execute, GccZone.valueOf(region));
    ...
    public Optional<String> describe(Resource resource, GccDescribeContextObject dco, String region) throws Exception {
        ...
        try {
            Compute.Disks.Get getDisk =
                    dco.getCompute().disks().get(gccCredential.getProjectId(), GccZone.valueOf(region).getValue(), resource.getResourceName());
