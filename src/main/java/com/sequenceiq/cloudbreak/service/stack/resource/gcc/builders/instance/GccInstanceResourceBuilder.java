...
public class GccInstanceResourceBuilder extends GccSimpleInstanceResourceBuilder {
    ...
    public List<Resource> create(GccProvisionContextObject po, int index, List<Resource> resources, TemplateGroup templateGroup, String region) throws Exception {
        ...
        listOfDisks.addAll(getBootDiskList(resources, gccCredential, GccZone.valueOf(region)));
        listOfDisks.addAll(getAttachedDisks(resources, gccCredential, GccZone.valueOf(region)));
        ...
        instance.setMachineType(String.format("https://www.googleapis.com/compute/v1/projects/%s/zones/%s/machineTypes/%s",
                po.getProjectId(), GccZone.valueOf(region).getValue(), gccTemplate.getGccInstanceType().getValue()));
        ...
        Compute.Instances.Insert ins =
                po.getCompute().instances().insert(gccCredential.getProjectId(), GccZone.valueOf(region).getValue(), instance);
    ...
    public Boolean delete(Resource resource, GccDeleteContextObject d, String region) throws Exception {
        ...
        try {
            ...
            Compute.ZoneOperations.Get zoneOperations = createZoneOperations(d.getCompute(), gccCredential, operation, GccZone.valueOf(region));
            Compute.GlobalOperations.Get globalOperations = createGlobalOperations(d.getCompute(), gccCredential, operation);
    ...
    public Optional<String> describe(Resource resource, GccDescribeContextObject dco, String region) throws Exception {
        try {
            ...
            return Optional.fromNullable(describe(stack, dco.getCompute(), resource, GccZone.valueOf(region)).toPrettyString());
        } catch (IOException e) {
